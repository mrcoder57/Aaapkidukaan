...
const Productcard = ({ id, title, price, description, category, image, rating }) => {
    return (
        <Tilt className='xs:w-[120px] w-[50%]'>
            <motion.div
        ...
        rounded-[20px] shadow-card ml-5 mb-3'
            >
               <div className="card bg-base-100 shadow-xl max-h-[600px] m-10 w-72 h-[500px]">
  <figure className=' object-cover'><img className=' object-cover'src={image} alt={title} /></figure>
  <div className="card-body" key={id}>
    ...
    <div className="card-actions justify-end">
      ...
      <div className="badge badge-outline"><span>₹{price*10}</span></div> 
      ...
      <>
        <div className=' h-32 grid grid-cols-1 md:grid-cols-3 '>
            {data.map(({ id, title, price, description, category, image, rating }) => (
                <Productcard key={id} image={image} title={title} description={description} price={price}
                />
            ))}
